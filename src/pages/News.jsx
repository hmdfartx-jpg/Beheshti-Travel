...
export default function News({ newsList, setPage, lang, viewId }) {
  const currentLang = lang === 'ps' ? 'ps' : 'dr';
  ...
  const handleCopyLink = () => {
    ...
    alert(currentLang === 'ps' ? "لینک کاپي شو!" : "لینک کپی شد!");
  };
  ...
  if (viewId) {
    ...
    if (!currentItem) return <div className="text-center py-20">خبر یافت نشد.</div>;

    return (
      <div className="max-w-4xl mx-auto px-4 py-10 font-[Vazirmatn]" dir="rtl">
        <button 
          onClick={() => setPage('news')}
          className="flex items-center gap-2 text-gray-500 hover:text-[#1e3a8a] mb-8 font-bold transition"
        >
          {currentLang === 'ps' ? <ChevronLeft size={20}/> : <ChevronRight size={20}/>}
          {t.back}
        </button>

        <article className="bg-white rounded-[2.5rem] shadow-xl border border-gray-100 overflow-hidden mb-12">
          {currentItem.image_url && (
            <img src={currentItem.image_url} className="w-full h-[450px] object-cover" alt={currentItem.title} />
          )}
          <div className="p-8 md:p-12">
            <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
              <div className="flex items-center gap-3 text-gray-400 text-xs font-bold bg-gray-50 px-4 py-2 rounded-full">
                ...
                <span dir="ltr">{new Date(currentItem.created_at).toLocaleDateString(currentLang === 'ps' ? 'ps-AF' : 'fa-IR')}</span>
                <span className="w-1.5 h-1.5 bg-gray-200 rounded-full mx-1"></span>
                <Tag size={16} className="text-[#1e3a8a]"/>
                <span>بهشتی تراول</span>
              </div>
            ...
            <h1 className="text-3xl md:text-5xl font-black text-gray-800 mb-8 leading-tight">{currentItem.title}</h1>
            <div className="text-gray-600 text-lg leading-10 text-justify whitespace-pre-wrap mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {relatedNews.map(item => (
                ...
                >
                  ...
                  <h4 className="font-black text-gray-800 text-sm line-clamp-2 leading-7 group-hover:text-[#1e3a8a] transition-colors px-2">{item.title}</h4>
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12 font-[Vazirmatn]" dir="rtl">
      ...
      <div className="grid grid-cols-1 gap-8">
        {sortedNews.length > 0 ? (
          sortedNews.map((item) => (
            <div key={item.id} className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 flex flex-col md:flex-row overflow-hidden hover:shadow-xl transition-all duration-500 group border-r-4 border-r-transparent hover:border-r-[#1e3a8a]">
              {item.image_url && (
              ...
              <div className="p-8 flex-1 flex flex-col">
                <div className="flex items-center gap-4 text-gray-400 text-[11px] mb-4 font-bold">
                  ...
                  <span dir="ltr">{new Date(item.created_at).toLocaleDateString(currentLang === 'ps' ? 'ps-AF' : 'fa-IR')}</span>
                </div>
                <h2 className="text-2xl font-black text-gray-800 mb-4 group-hover:text-[#1e3a8a] transition-colors leading-tight line-clamp-2 cursor-pointer" onClick={() => setPage(`view-news-${item.id}`)}>{item.title}</h2>
                <p className="text-gray-500 leading-8 text-sm text-justify line-clamp-2 mb-6">{item.description}</p>
                <div className="mt-auto pt-4 border-t border-gray-50">
                   ...
                   >
                      {t.readMore}
                      {currentLang === 'ps' ? <ArrowLeft size={16}/> : <ArrowRight size={16}/>}
                   </button>
                </div>
              </div>
            </div>
