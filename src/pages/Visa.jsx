...
export default function Visa({ t, lang, setPage }) {
  ...
  const getBackText = () => {
      ...
      if (lang === 'ps') return 'شاته';
      return 'بازگشت';
  };

  const getSubTitle = () => {
      if (lang === 'en') return "Visa services with lowest rejection risk";
      if (lang === 'ps') return "د ویزو ترلاسه کول په خورا ډاډ سره";
      return "خدمات اخذ ویزا با کمترین ریسک ریجکتی";
  };

  const getDocsLabel = () => {
      if (lang === 'en') return "Required Documents";
      if (lang === 'ps') return "اړین اسناد";
      return "مدارک لازم";
  };
  ...
  if (selectedCountry) {
    // فرم درخواست ویزا
    return (
      <div className="max-w-xl mx-auto bg-white p-12 rounded-[3rem] shadow-2xl animate-in zoom-in duration-300" dir={isLtr ? 'ltr' : 'rtl'}>
        <button onClick={() => setSelectedCountry(null)} className="mb-6 text-gray-400 flex items-center gap-2 hover:text-[#058B8C]">
          {isLtr ? <ArrowLeft size={18}/> : <ArrowRight size={18}/>} 
          {getBackText()}
        </button>
        ...
        <form className="space-y-6" onSubmit={handleSubmitRequest}>
          <div className="space-y-2">
            <label className={`text-xs font-black text-gray-400 ${isLtr ? 'ml-2' : 'mr-2'}`}>{t.common.name}</label>
          ...
          <div className="space-y-2">
            <label className={`text-xs font-black text-gray-400 ${isLtr ? 'ml-2' : 'mr-2'}`}>{t.common.phone}</label>
  ...
  // لیست ویزاها
  return (
    <div className="space-y-12 animate-in slide-in-from-bottom-8" dir={isLtr ? 'ltr' : 'rtl'}>
          ...
          <div key={key} className="bg-white rounded-3xl p-8 border hover:border-[#D4AF37] transition-all group">
            ...
            <div className="space-y-4">
              <div className="bg-gray-50 p-4 rounded-xl min-h-[100px]">
                <p className="text-[10px] font-black text-gray-400 uppercase mb-2">{getDocsLabel()}</p>
