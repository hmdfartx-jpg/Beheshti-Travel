...
export default function Admin({ news, bookings, settings, onUpdate, setPage }) {
  // --- مدیریت لاگین ---
  ...
  const handleLogin = (e) => {
    ...
    // نام کاربری و رمز عبور ثابت (قابل تغییر)
  ...
  // ---------------------------------------------------------
  // 🔴 اگر کاربر لاگین نکرده باشد، این صفحه نمایش داده می‌شود
  // ---------------------------------------------------------
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4 font-[Vazirmatn]" dir="rtl">
        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden border border-gray-200 animate-in zoom-in-95 duration-300">
          ...
          <div className="bg-[#1e3a8a] p-8 text-center relative overflow-hidden">
            ...
            <h2 className="text-2xl font-black text-white">ورود به پنل مدیریت</h2>
            <p className="text-blue-200 text-sm mt-2">امنیت بالا، مدیریت آسان</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="p-8 space-y-5">
            ...
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-1">کد امنیتی</label>
            ...
            <button type="submit" className="w-full bg-[#1e3a8a] hover:bg-blue-900 text-white py-3.5 rounded-xl font-bold transition shadow-lg shadow-blue-200 flex items-center justify-center gap-2 mt-4">
               ورود به سیستم <ArrowRight size={18} className="rotate-180"/>
            </button>
  ...
  // --- اگر لاگین موفق بود، بقیه پنل نمایش داده می‌شود ---
  ...
  const handleSubmitNews = async (e) => {
    ...
    if (editingId) {
      ...
      const { error } = await supabase.from('news').insert([{ title: newNews.title, description: newNews.desc, image_url: newNews.img, pinned: false, created_at: currentDate }]);
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden min-h-[600px] font-[Vazirmatn]">
          ...
          {activeTab === 'news' && (
             <div className="space-y-6">
                <form onSubmit={handleSubmitNews} className="p-4 border rounded-xl bg-gray-50 space-y-3">
                   <input placeholder="تیتر خبر" value={newNews.title} onChange={e=>setNewNews({...newNews, title: e.target.value})} className="w-full p-2 rounded border"/>
                   ...
                   <textarea placeholder="متن کامل خبر..." value={newNews.desc} onChange={e=>setNewNews({...newNews, desc: e.target.value})} className="w-full p-2 rounded border h-24"/>
                   <button className="bg-blue-600 text-white px-6 py-2 rounded-lg font-bold">{editingId ? 'ذخیره تغییرات' : 'انتشار خبر جدید'}</button>
                </form>
                <div className="grid gap-3">
                  {news.map(n => (
                    <div key={n.id} className="flex gap-4 p-3 border rounded-xl items-center bg-white">
                       <img src={n.image_url} className="w-16 h-16 rounded object-cover bg-gray-100"/>
                       <div className="flex-1">
