...
export default function Tracking({ t, lang }) {
  ...
  const getSubtitle = () => {
    if (lang === 'en') return "Enter your tracking code";
    if (lang === 'ps') return "خپل تعقیب کوډ دننه کړئ";
    return "کد پیگیری خود را وارد کنید";
  };

  const getNotFoundText = () => {
    if (lang === 'en') return "Not Found";
    if (lang === 'ps') return "ونه موندل شو";
    return "یافت نشد";
  };

  return (
    <div className="max-w-xl mx-auto space-y-8 py-10" dir={isLtr ? 'ltr' : 'rtl'}>
      ...
      <div className="space-y-4 mt-10">
        {trackResult?.map((res, i) => (
          <div key={i} className="bg-white p-8 rounded-[2rem] shadow-sm border flex items-center justify-between">
            <div>
              {/* تلاش برای ترجمه نوع درخواست، در غیر این صورت خود متن */}
              <h4 className="font-black text-xl">{t.nav[res.type] || res.type.replace('_', ' ')}</h4>
        ...
        {trackResult && trackResult.length === 0 && <p className="text-center text-red-500 font-bold">{getNotFoundText()}</p>}
      </div>
    </div>
  );
}